...
/**
 * H0 — capability probe. PHY / advertising flags, barometer presence, and the
 * *actual* runtime grant state of the four permissions (re-checked on resume).
 */
...
fun ProbeScreen() {
    ...
    var permissionState by remember { mutableStateOf(readPermissionState(context)) }

    val lifecycleOwner = LocalLifecycleOwner.current
    DisposableEffect(lifecycleOwner) {
        val observer = LifecycleEventObserver { _, event ->
            if (event == Lifecycle.Event.ON_RESUME) permissionState = readPermissionState(context)
        }
...
private fun SectionTitle(text: String) {
    Text(text, fontWeight = FontWeight.Bold, fontSize = 16.sp, modifier = Modifier.padding(top = 4.dp, bottom = 2.dp))
}
...
private fun KeyValue(key: String, value: String) {
    Row(modifier = Modifier.fillMaxWidth()) {
        Text(key, modifier = Modifier.padding(end = 12.dp), fontSize = 14.sp, fontWeight = FontWeight.Medium)
